...
class IndexManager:
    ...
    def buscar(self, termino, compañeros=None, años=None, extensiones=None, carpetas=None, clientes=None, proyectos=None, ordenes=None, props_fabricacion=None, props_bandas=None, material=None, tratamiento=None, espesor=None, solo_placa_ce=False):
        ...
        if solo_placa_ce:
            base_where += (
                " AND SUBSTRING(UPPER(nombre_archivo) FROM"
                " '^[A-Z0-9]+(?:-[0-9]+)?(?:\\.[0-9]{1,4}){0,2}\\.E[0-9]+')"
    ...
    def obtener_propiedades_contexto(self, compañeros=None, años=None, clientes=None, proyectos=None):
        """V2.0.2 - Cascada de filtros de propiedades: valores de Material,
        ...
        Proyectos. Una sola pasada de tabla con GROUPING SETS (~0,2-0,6 s);
        SIEMPRE se llama desde un hilo de fondo, nunca desde el de UI.
        ...
        try:
            ...
            for fila in cursor.fetchall():
                ...
                for i, clave in enumerate(claves):
                    if fila[i]:
                        res[clave].add(str(fila[i]).strip().upper())
            return res
        except Exception as e:
